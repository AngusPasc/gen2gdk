...
type
  ...
  public
    ...
    procedure SavePack(const FileName: String);
    procedure LoadPack(const FileName: String);
...
begin
  ...
  begin
    ...
    begin
      str := G2Param(ExtractFileName(Form1.od1.Files[i]), '.', 0);
      f := NewFile(fld, str);
...
begin
  mc := App.PlugInput.MousePos;
  if PtInRect(RectSave, md) and PtInRect(RectSave, mc) then
  begin
  ...
  if PtInRect(RectSaveAs, md) and PtInRect(RectSaveAs, mc) then
  begin
    if Form1.sd2.Execute then
    App.SavePack(Form1.sd2.FileName);
    App.FreezeInputTime := GetTickCount;
  end;
  if PtInRect(RectLoad, md) and PtInRect(RectLoad, mc) then
  begin
...
//TMyApp BEGIN
procedure TMyApp.SavePack(const FileName: String);
type
...
procedure TMyApp.LoadPack(const FileName: String);
type
  THeader = packed record
    Definition: String[4];
    Version: DWord;
